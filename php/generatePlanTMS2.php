...
if (preg_match("#^\d{2}/\d{2}/\d{4}#sm", $input, $date)) {
    ...
    if($date==$today) {
        ...
        preg_match_all("#^\d+\t (PostFlight|Authorized|Current|Cancelled)\t([AB]\d{3}S? ?.?[1A-Z]?|[AB]IPTS?\d{3} ?[A-Z]?|M2-\d{3}[AF]-?S? ?[A-Z]?)\t (VH-\w{3}|F141-G|F242-G)\t (\d{2}:\d{2})\t (\w+ ?-?/?\w* ?-?/?\w* ?-?/?\w* ?-?/?\w* ?-?/?\w* ?-?/?\w*)\t (\w+ ?-?/?\w* ?-?/?\w* ?-?/?\w* ?-?/?\w* ?-?/?\w* ?-?/?\w*)?#sm", $input, $matches);
